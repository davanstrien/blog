...
One note here: this is clearly a nice dream for some applications of ml but others might be more 'one off tools' where this might be less important. For example tools developed to help work with the ingest of a new collection.

## Infastuctire buckets

A bunch of tools that help get closer to this dream, these broadly fall into three categories 
...
There is a slightly overwhelming range of tools here. I think some of these only make sense at a certain level of scale i.e. auto-scaling inference accross multiple regions is probably not a scenerio many (any?) glam instituions are dealing with at the moment for their ml pipelines. There are probably some notable exceptiosn here; internet archive, wikipedia etc.
...
Two major compute needs: development and training/evaluation. 

The development one intended to make experimentation easy. Do basic training etc. 

Training compute: multi gpus to do parameter sweeps etc. I think financial constrains often make this part less possible in a GLAM setting. There may also be some benefits to restricting compute slightly. Since GLAMs might (maybe should) be also considering how to share models approaches it migth not be helpful to immediately jump to models which require multiple GPUs to fine-train when a model with much more constrained resources might only be slightly worse. 
...
- New architecture every year: Kepler -> pascal -> volta -> turing -> ampere
- Server and then 'enthusiast', then consumer versions of the cards
- Business only supposed to use primarilyserver cards. I don't know what the definition of business is here but it seems this is reguarly ignored 

A note here; I think a lot of use cases for deep learnings in GLAMs could probably be well addressed by a dev box with one 3090 card and a bunch of storage. This may also be slighly less annoying for IT compared to going into the cloud.

Cost of cloud gets expensive compared to local deep learning rig. 
...
Managing enviroments, resources
...
Deep learning frameworks put on chart with good for production and good for development chart. 

TODO add chart

I think in the context of using deep learning in a GLAM setting there should be a lot of emhasis placed on using frameworks that focus on making things easier to understand for a broader audience. I personally think GLAMs should be trying to upskill a broad range of staff with some of the skills associated with ml and a core of staff with the skills to develop all/some/most things themselves as *domain expersts*. For this to work I think there needs to be an emphasis placed on the accessibilty and clenaess of model developmenet. i.e. prefer to use a framework training loop i.e. fastai/keras/lightning rather than roll your own since this probably won't add much but will often make code harder for others to understand. 

## Experiment tracking

Deep learning usually involves at least some experimentation with different model architectures, data augmentations, hyper paramters etc. Keeping track of this can be a pain to do manually. 

Various options available. Weights and Biases seems to be most liked commercial offering. There are a bunch of open source projects that do this too. 

## Hyperparmater Optimization

Various options for doing this including local/open source offerings. 

Increasingly a move towards 'end-to-end' platforms for helping accross the ml lifecyle. 
